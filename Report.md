...
// Method to read Sudoku solution from file and initialize Sol array
...
The mssv.c program uses a multithreaded program to ensure proper access to shared resources when validating the sudoku solution. It uses synchronizing techniques to achieve its purpose.

The program uses mutex locking to ensure mutual exclusion when accessing shared resources like 'counter' variable or arrays like 'Row','Col','Sub'. There are methods setRowValue, setColValue, setSubValue, updateCounter which locks the mutex before accessing shared resources and also unlocks after use to prevent race conditions.

It uses conditional variables pthread_cond_t cond to make thread 2,3,4 wait for completion of first three threads using pthread_cond_wait.After completing its task the threads signals its completion by incementing threads_finished and using pthread_cond_signal to wake up the next threead.

Thread 1,2,3 validate the rows and subgrids and 4 validates the columns. With validateRegion each thread has its assigned regions which it uses to validate the suodku solution.
Each thread has the parameter 'delay' to simulate potential synchorization issues. 
...
When this solution is tested.The program gives the following ouput:
"
Thread ID-1 139959156598336: valid
Thread ID-2 139959148205632: valid
Thread ID-3 139959139812928: valid
Thread ID-4 139959129339456: valid
Thread ID-4 is the last thread
There are toal 27 valid rows,columns and sub-grids,and the solution is valid.
"
...
Thread ID-1 139770632070720: row 1, sub-grid 3, are invalid
Thread ID-2 139770623678016: row 6, sub-grid 4, are invalid
Thread ID-3 139770615285312: row 8, sub-grid 9, are invalid
Thread ID-4 139770604811840: column 1, column 7, column 8,  is invalid
Thread ID-4 is the last thread
There are toal 18 valid rows,columns and sub-grids,and the solution is invalid.